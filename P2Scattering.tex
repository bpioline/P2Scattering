...
\mathematica{1.0}{ <<P2Scattering`}{P2Scattering 1.5 -- A package for evaluating DT invariants on $K_{P^2}$}
...
reproducing the GV invariant $N_3^0=27$. It should be stressed that the current implementation of the routine \fun{ScattIndex}
assumes that the index associated to each tree is a product of Kronecker indices associated to each
vertex, and may give the wrong result if some of the edges carry non-primitive charges. In the case above, it does produce the correct results for both trees $K_3(3,3)+K_{12}(1,1)$. 
{\bf As of version 1.5,  the correct way to compute the index in presence of non-primitive intermediate states is to use the routine \fun{ScattIndexImproved}, developed by Rishi Raj.
In order to determine the trees contributing to a given charge vector at a given point $(s,t)$, it is also more efficient to use the new routines \fun{ConstructLVDiagram} and \fun{LVTreesFromListRays}.
}

\medskip

Similarly, one can find the trees contributing near
...
{\bf As of version 1.5, the correct way to compute the index in presence of non-primitive intermediate states is to use \fun{McKayScattIndexImproved}. In order to determine the trees contributing to a given dimension vector in the anti-attractor chamber, it is also more efficient to use the new routines \fun{ConstructMcKayDiagram} and \fun{McKayTreesFromListRays}.}
...
The first version of this package was released together with the preprint \cite{Bousseau:2022snm}. 
...
\defn{LVTrees}{\vardef{r},\vardef{d},\vardef{chi}}{gives the list of precomputed trees at large volume, when available}
...
\defn{MeijerT}{\vardef{z}}
{computes $T(z)$ using MeijerG function representation, valid for $\Im z>0$}
\defn{MeijerTD}{\vardef{z}}
{computes $T_D(z)$ using MeijerG function representation, valid for $\Im z>0$}
\defn{tauFromz}{\vardef{z}}
{computes $\tau(z)$ using ratio of hypergeometric functions, valid for $\Im z>0$}
\defn{zFromtau}{\vardef{z}}{computes $z=-1/(J_3(\tau)+15)$}
...
{ sorts trees by decreasing wall radius}
...
{constructs consistent trees from constituents in \var{LiCh} of the form $k_i Ch[m_i]$, 
which are stable at $(s_0,t_0)$ after perturbing $\ch_2$ for each  $Ch[m_i]$}
...
{ searches possible list of constituents  with slope in $[mmin,mmax]$, number of constituents in $\var{[nmin,nmax]}$, electric potential less than \var{phimax} and charges adding up to $[r,d,\chi)$}
...
and $phimax=\varphi_{s}(\gamma)=d-rs$ }

\defn{ScanBinarySplits}{\{\vardef{r},\vardef{d},\vardef{chi}\},\vardef{t0}}
{produces list of $[r',d',\chi')$ such that $[r,d,\chi)$ can split into $[r',d',\chi')$ along the ray 
$\cR_0(\gamma)$ starting at height $t_0$}
...
\defn{ConstructLVDiagram}{\vardef{smin},\vardef{dmax},\vardef{phimax},\vardef{Nmax},\vardef{ListRays}}{Constructs the 
large volume scattering diagram truncated up to $\varphi\leq \phi_{max}$, 
starting from initial rays $\pm \cO(k)$
in the range $s_{min}\leq k \leq s_{max}$, with scattering products $N_1\gamma_1+N_2\gamma_2$, $2\leq N_1+N_2\leq N_{max}$ at each vertex.  The output 
consists of a list of  $\{ (r,d, \chi), \{x,y\}, p_1, p_2,n_1,n_2, d \}$ where
$(u,v)$ is the initial point for the ray emitted from $n_1$ copies of $p_1$-th ray and $n_2$ copies of 
 $p_2$-th ray at depth $d=d_1+d_2+1$ (with $p_1=p_2=d_1=d_2=0$ for initial rays);
 If \var{ListRays} is not empty, then use it as initial rays.}
 
\defn{LVTreesFromListRays}{\vardef{ListRays},\{\vardef{r},\vardef{d},\vardef{chi}\}}{ extracts the list 
of distinct trees with given Chern character}


\defn{ScattIndexImproved}{\vardef{TreeList}}{ computes the index for 
each tree in \var{TreeList}, taking care of non-primitive internal states}

\defn{KroneckerTabs}{\vardef{m},\vardef{Nmax}}{gives the list of populated dimension vectors $\{N_1,N_2\}$ for Kronecker  quiver with  $m$ arrows, with $0<N_1+N_2<
N_{max}$, restricting to $(N_1,N_2)$ coprime.}

\defn{CostPhi}{\{\vardef{r},\vardef{d},\vardef{chi}\},\vardef{s}}{$=d-r s$}
...
\defn{ConstructMcKayDiagram}{\vardef{phimax},\vardef{ListRays}}{Constructs the 
orbifold quiver  scattering diagram with height up to \var{phimax}. The output 
consists of a list of  $\{ (n_1,n_2,n_3), \{u,v\}, p_1, p_2,n_1,n_2 \}$ where
$(u,v)$ is the initial point for the ray emitted from $n_1$ copies of ray $p_1$ and $n_2$ copies of ray $p_2$;
 If \var{ListRays} is not empty, then use it as initial rays."}
 
\defn{McKayTreesFromListRays}{\vardef{ListRays},\vardef{Nvec}}{ extracts the list 
of distinct trees with given dimension vector}


\defn{McKayScattIndexImproved}{\vardef{TreeList}}{ computes the index for 
each tree in \var{TreeList}, taking care of non-primitive internal states}

\defvar{InitialRaysOrigin}{is a List of initial points $(u_i,v_i)$ 
for the 3 initial rays in the orbifold scattering diagram}
...
\defn{BinaryTreeIndex}{\vardef{TreeList}}{computes the index of each binary tree in \var{TreeList}}

\defn{BinaryTreeIndexInternal}{\vardef{Tree}}{produces a list of wall-crossing factors from each vertex in a binary tree}

\defn{AbelianizedScatteringSequence}{\vardef{ScattSeq}}{produces a list of abelianized constituents, by replacing each $k Ch[m]$ by $\{k_i Ch[m]\}$ where $k_i$ runs over integer partitions of $k$}

\defn{IndexFromAbelianizedSequences}{\vardef{LiAbelianized},\{\vardef{s0},\vardef{t0} \}}{constructs all possible perturbed binary trees from constituents in \var{LiAbelianized} which are stable for 
the large volume central charge at $(s_0,t_0)$, and compute the contribution to the index from each of them}

\defn{IndexFromSequences}{\vardef{LiScattSeq},,\{\vardef{s0},\vardef{t0} \}}{constructs all possible lists of abelianized constituents from the list of scattering sequences \var{LiScattSeq}, removes duplicates, and applies \fun{IndexFromAbelianizedSequences} on each of them}
...
In addition, the package includes routines adapted from the Mathematica 
...
The names of the routines are prefaced by {\tt P2} to avoid conflicts.



\defn{P2HiggsBranchFormula}{\vardef{Mat},\vardef{Cvec},\vardef{Nvec} }
	                                   {computes the refined index  of a quiver with DSZ products $\alpha_{ij}=\var{Mat}[[i,j]]$, 
...
\defn{P2RationalInvariant}{\vardef{Mat},\vardef{Cvec},\vardef{Nvec},\vardef{y} }{computes the refined rational index
                  of a quiver with DSZ matrix $\alpha_{ij}=\var{Mat}[[i,j]]$,                  
                  FI parameters $\zeta_i=\var{Cvec}[[i]]$,  dimension vector $N_i=\var{Nvec}[[i]]$, 		using Reineke's formula}


\defn{P2StackInvariant}{\vardef{Mat},\vardef{Cvec},\vardef{Nvec},\vardef{y} }{computes the 
stacky invariant
                  of a quiver with DSZ matrix $\alpha_{ij}=\var{Mat}[[i,j]]$,                  
                  FI parameters $\zeta_i=\var{Cvec}[[i]]$,  dimension vector $N_i=\var{Nvec}[[i]]$, 		using Reineke's formula}
...
\defn{P2QDeformedFactorial}{\vardef{n},\newdefy }{computes the $q$-deformed factorial $[n,y]!$}
